...
当槽位刚进入待批准或未读颜色时，所有设备都会蜂鸣三声；状态未变化则不会重复
提示，两者同时出现时以待批准优先。蜂鸣由非阻塞状态机排程，Navigate 页面的
静音开关在 Core2/CoreS3 上保存在 `codexcore` NVS 命名空间，StopWatch 上保存在
`stopwatch` 命名空间。
...
Command 控件采用两列三行网格，Navigate 页面采用圆形虚拟摇杆和触摸旋钮。
黄色/蓝色实体键
（`M5.BtnA`/`M5.BtnB`，GPIO 2/1）使用 350 ms 双击窗口。黄色键单击轻触右 Alt
触发 Mic，双击切换到上一页；蓝色键单击发送 Enter，双击切换到下一页。
CST820 触摸屏负责所有屏幕动作。

横向滑动至少需要 70 像素，且水平位移必须大于垂直位移。左滑前进一页，右滑
返回一页。触摸动作延迟 90 ms 才发送按下报告；移动超过 16 像素会取消候选控件，
因此从按钮上开始滑动也不会误发 HID 动作。快速点击则在触摸释放时发送完整的
按下/释放报告。
...
468 x 468 设计坐标缩放。Agent Key、Command Key 和旋钮中心会发送按下与
释放事件。所有设备的虚拟摇杆都捕获控件内部拖动，并以最多每 32 ms 一次的
频率上报归一化角度与距离；旋钮外环将角位移累计为每圈 24 个编码器步进，
中心仍保留普通按下/释放和主机端长按行为。横向滑动可切换页面，Core2/CoreS3
阈值为 50 像素，StopWatch 为 70 像素。Core2 和 StopWatch 会以 35 ms 震动确认
有效触摸；CoreS3 没有马达，改为在独立扬声器通道上播放 12 ms 哔嗒声，Navigate
页面的静音开关会同时关闭该声音和状态提示音。
...
StickS3 将实体按键按下或任意 Agent 灯光状态的实际变化视为活动。无活动 1 分钟
后，屏幕亮度从 120 降至 20；活动会立即恢复亮度并重新开始两个计时器。连续
30 分钟无活动后，固件调用 M5Unified 的硬件关机路径；再次按电源键可开机。
