...
单击黄色实体键触发 Mic，在 350 ms 内双击切换到上一页；单击蓝色实体键触发
Enter，双击切换到下一页。Agent、Command、方向和旋钮操作仍使用触摸，并针对
圆形安全区域重新排布。屏幕任意位置左滑切换到下一页、右滑切换到上一页；滑动
起点下的控件会被取消，不会误触。有效触摸、滑动和实体按键动作会提供短促震动
确认。
...
所有设备都提供连续虚拟摇杆、触摸旋钮和 `ALERT`/`MUTED` 开关：

- 向任意方向拖动虚拟摇杆，固件会连续上报角度和距离，包括中心到外环之间的
  部分行程。
- 沿旋钮外环拖动，模拟顺时针或逆时针编码器步进。
- 按下或按住旋钮中心，模拟普通旋钮按下和 500 ms 设置操作。
- `ALERT`/`MUTED` 开关控制下面的状态提示音，选择会保存在设备上。

当任意 Agent 进入等待批准状态，或执行完成产生未读结果时，固件会蜂鸣三声。
只有在状态刚发生变化时才会响，主机重复推送同一状态不会重复提示。
